...
def convert_to_primary_centric(paramsDF, objectNames, numobjects, sample_num):
    ...
    for i in range(len(objectNames)-1):
        ...
        deltaRA_1 = np.array(paramsDF['Delta-RA']).astype(np.float)
        deltaDEC_1 = np.array(paramsDF['Delta-DEC']).astype(np.float)
    
        RA_1_err = np.array(paramsDF['Delta-RA-err']).astype(np.float)
        DEC_1_err = np.array(paramsDF['Delta-DEC-err']).astype(np.float)
    ...
    updatedDF.to_csv(objectNames[0]+'/'+objectNames[0]+'_obs_df.csv')
