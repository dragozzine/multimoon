...
results = np.loadtxt("resultfiles.txt", delimiter = "\n", dtype = "str")
rows = results.copy()
...
table = pd.DataFrame(index = rows, columns = ["lhood","reduced_chi_sq","mass_tot","sma","ecc","aop","inc","lan","mea","mass_1","mass_2","sma","ecc","aop","inc","lan","mea"])
...
table = table.transpose().copy()
table.to_csv("../results/scraped_results.csv")
