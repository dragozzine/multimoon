...
def mm_autorun(sampler, essgoal, state, initsteps, maxiter, verbose, objname, p0, runprops):
	...
	if runprops.get('nburnin') == 0:
		if runprops.get("chain_file") != None:
			state = sampler.run_mcmc(None, initsteps, progress = True, store=True, thin = 10)
		else:    
			state = sampler.run_mcmc(p0, initsteps, progress = True, store=True, thin = 10)
	else:
	...
	while (50*iat > ngens):
		...
		state = sampler.run_mcmc(state, moresteps, progress = True, store=True, thin = 10)
		if verbose:
			print(initsteps, " steps have been completed.")
			print("Calculating the effective sample size.")
	...
	if int(nadditional + ngens) > maxiter:
		...
		state = sampler.run_mcmc(state, nadditional, progress = True, store=True, thin = 10)
		iat = autocorrelation(sampler,objname, filename = "_" + str(counter))
		counter += 1
		ngens = sampler.get_chain().shape[0]
		if verbose:
